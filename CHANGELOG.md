...
### Changed
- `src/services/session.service.js` — added `appendEvent()` method; `createSession()` now records `session_started`; `ensureActiveSession()` records `session_auto_created` on auto-create; `appendTurn()` records `conversation_turn_created`; `createSession()` and `ensureActiveSession()` wrap internal `appendEvent` calls in try/catch for resilience (#7); added `reactivateSession()` method and `limit` option for `getTurns()` (#6).
- `src/sockets/dataHandler.js` — added optional `options.sessionService` constructor parameter for dependency injection. All internal `sessionService` references changed to `this.sessionService`. This is backward compatible — existing code passes no options and falls through to the default singleton (#7). Added `handleRestoreSession()` method and `restore_session` Socket.IO listener (#6).
- `src/sockets/InterviewTranscriptBuffer.js` — added `clear()` and `hydrateFromTurns()` methods for Session restore support (#6).
